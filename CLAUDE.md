...
## Python environment
- Python 3.11+ (pyproject.toml `requires-python`), venv at .venv/
- Always use .venv/bin/python and .venv/bin/pip
- Install: pip install -e . (pyproject.toml, hatchling build system, sole source of truth
  for pinned dependency versions — don't duplicate version numbers here, they drift)
...
- data/chroma/ is generated by scripts/build_knowledge_base.py and gitignored —
  never commit it. netagent.sh start/restart auto-builds it if missing, but
...
## Running the app
./netagent.sh start|stop|restart|status|logs   # always use netagent.sh — handles PID, child cleanup, nohup
...
- scripts/install_service.sh — installs+enables a systemd --user service
  ...
  netagent.sh auto-detects this service and delegates start/stop/restart/status
